...
const list = (title => {
  if (document.querySelector('#list')) return document.querySelector('#list')
  ...
  <a href="/bruh-beagle-upgrade-maker/">+</a>`) // the html in the details
class Upgrade {
  constructor () {
    ...
    button.addEventListener('click', () => { // when the user clicks it
      if (increase() >= this.price) { // if they can buy it
  ...
  get price () { // this is where upgrades can see the price
    if (isNaN(+this.html.querySelector('span').innerText)) return this.meta().startprice
