/* global localStorage, EventTarget, Event */
...
class Upgrade extends EventTarget {
  constructor (
    ...
    price.innerText = BigInt(localStorage.getItem(title.innerText.replace(/ /g, '')) || 0n) || BigInt(startprice) // add the price
  ...
  get price () {
    ...
    return BigInt(this.html.querySelector('span').innerText) // return the price innerText
  }

  set price (amount) {
    // this is how upgrades change the price
    this.html.querySelector('span').innerText = BigInt(amount) // change the innerText to a number
  }
